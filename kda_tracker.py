...
Структура CSV:
    match_id, game_id, game_num, team, player, kills
...
FIELDNAMES = ["date", "tournament", "game_num", "team", "player", "champion", "kills", "match_id", "game_id"]


class KdaRow:
    __slots__ = ("match_id", "game_id", "game_num", "team", "player", "champion", "kills", "tournament", "date")

    def __init__(self, match_id, game_id, game_num, team, player, champion, kills, tournament="", date=""):
...
def extract_kda_rows(
    ...
    for game_num, game_id in enumerate(all_game_ids, 1):
        ...
        for p in sorted(participants, key=lambda x: x.get("participantId", 0)):
            ...
            player_rows.append(KdaRow(match_id, game_id, game_num, team_name, name, champ, kills, tournament=tournament, date=game_date))

        for team_name, total_kills in team_kills.items():
            rows.append(KdaRow(match_id, game_id, game_num, team_name, "", "", total_kills, tournament=tournament, date=game_date))
